...
RSpec.describe Cycad::Transactions do
  let(:transaction1) do
    double(Cycad::Transaction,
  ...
  let!(:transactions) { Cycad::Transactions.new(repo_transactions) }
  ...
  context '.income_only' do
    it 'returns transactions with an amount greater than 0' do
      filtered = transactions.income_only
      expect(filtered).to contain_exactly(
        transaction1,
        transaction3,
        transaction4
  ...
  context '.expenses_only' do
    it 'returns transactions with an amount less than 0' do
      filtered = transactions.expenses_only
      expect(filtered).to contain_exactly(
        transaction2,
        transaction5
      )
  ...
  context '.amount_range' do
    it 'returns transactions within an amount range' do
      filtered = transactions.amount_range(50, 400)
      expect(filtered).to contain_exactly(
        transaction1,
        transaction3,
        transaction4
  ...
  context '.greater_than' do
    it 'returns transactions with amounts greater than or equal to X' do
      filtered = transactions.greater_than(-47)
      expect(filtered).to contain_exactly(
        transaction1,
        transaction3,
        transaction4,
  ...
  context '.less_than' do
    it 'returns transactions with amounts less than or equal to X' do
      filtered = transactions.less_than(200)
      expect(filtered).to contain_exactly(
        transaction1,
        transaction2,
        transaction4,
  ...
  context '.category' do
    it 'returns only transactions of that category' do
      filtered = transactions.category(1)
      expect(filtered).to contain_exactly(
        transaction2,
        transaction3
      )
  ...
  context 'when methods are chained' do
    it 'performs multiple filters on a set of transactions' do
      filtered = transactions.greater_than(-47).category(7)
